...
def preprocess_image(image):
    ...
    threshold_image = cv.adaptiveThreshold(
        blurred_image, 255, cv.ADAPTIVE_THRESH_GAUSSIAN_C, cv.THRESH_BINARY_INV, 3, 2)
    dilatation_size = 2
    dilation_element = cv.getStructuringElement(
        cv.MORPH_RECT, (2*dilatation_size + 1, 2*dilatation_size+1), (dilatation_size, dilatation_size))
...
def find_numbers(image, bounds):
    ...
    for contour in contours:
        ...
        if cv.arcLength(contour, True) > contour_perim_length:
